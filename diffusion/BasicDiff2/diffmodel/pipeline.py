...
class Pipeline:
    ...
    def run(
        ...
        with VRAMPeakMonitor("applying condition"):
            ### Change the condition here. ###
            cond_img = lq_tensor
            ### Change the condition here. ###
        
        if cond_img.shape[-1] < 512 or cond_img.shape[-2] < 512:
...
class SRNetPipeline(Pipeline):
    ...
    def __init__(
        ...
        super().__init__(cldm, diffusion, cond_fn, device)
    ...
    def apply_cleaner(
        ...
        if tiled and (lq.size(2) < tile_size or lq.size(3) < tile_size):
            print("[SRNet]: the input size is tiny and unnecessary to tile.")
            tiled = False
