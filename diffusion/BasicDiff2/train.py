...
from diffmodel.model import ControlLDM, Diffusion
from diffmodel.utils.common import instantiate_from_config, to, log_txt_as_img
from diffmodel.sampler import SpacedSampler
    ...
    while global_step < max_steps:
        ...
        for batch in loader:
            ...
            with torch.no_grad():
                ...
                cond_feature = lq
                ### Process the condition here. ###
                cond = pure_cldm.prepare_condition(cond_feature, prompt)
                # noise augmentation
            ...
            if global_step % cfg.train.image_every == 0 or global_step == 1:
                N = 8
                log_cond_feature = cond_feature[:N]
                ...
                with torch.no_grad():
                    ...
                    if accelerator.is_main_process:
                        for tag, image in [
                            ...
                            ("image/gt", (log_gt + 1) / 2),
                            ("image/lq", log_lq),
                            ("image/condition", log_cond_feature),
                            (
